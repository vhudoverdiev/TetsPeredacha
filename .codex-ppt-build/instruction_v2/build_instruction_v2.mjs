...
const FINAL_PPTX = path.join(OUTPUT_DIR, "CRM Передача - инструкция v8.pptx");
...
function addHeader(slide, num, section, title, subtitle = "") {
  ...
  addText(slide, title, 86, 132, 1050, 62, { size: 36, bold: true, color: C.text });
  if (subtitle) addText(slide, subtitle, 88, 205, 1060, 34, { size: 19, color: C.muted });
...
function addStepRow(slide, n, title, body, x, y, w = 1090) {
  ...
  addText(slide, title, x + 74, y + 8, 330, 22, { size: 18, bold: true });
  addText(slide, body, x + 74, y + 34, w - 104, 20, { size: 14, color: C.muted });
}

function addTip(slide, title, body, y, fill = C.greenSoft) {
  addShape(slide, "roundRect", 88, y, 1090, 58, fill, C.greenLine, 18);
  addText(slide, title, 116, y + 10, 320, 24, { size: 20, bold: true, color: C.greenDark });
  addText(slide, body, 420, y + 13, 720, 22, { size: 16, color: C.muted });
}

function addFeature(slide, n, title, where, result, x, y) {
  addShape(slide, "roundRect", x, y, 500, 104, C.white, C.greenLine, 18);
  addShape(slide, "ellipse", x + 20, y + 27, 50, 50, C.green);
  addText(slide, String(n).padStart(2, "0"), x + 20, y + 41, 50, 18, { size: 15, bold: true, align: "center" });
  addText(slide, title, x + 88, y + 14, 370, 38, { size: 20, bold: true });
  addText(slide, where, x + 88, y + 60, 370, 18, { size: 14, bold: true, color: C.greenDark });
  addText(slide, result, x + 88, y + 80, 380, 16, { size: 12, color: C.muted });
}
...
// 2
{
  ...
  const items = [
    ...
    [6, "Выдача задач", "Выдача задач → выбрать замечания", "Назначение сотруднику"],
    [7, "Квартиры", "Квартиры → карточка", "Фильтры и статусы"],
    [8, "Отчет", "Отчет", "Сводка по объекту"],
    [9, "Расход материалов", "Расход материалов", "Списание на работу"],
    [10, "Замеры", "Замеры", "Невыполненные сверху"],
    [11, "Почта и телефон", "Аккаунт → Профиль", "Контакты"],
    [12, "Подключение 2FA", "Аккаунт → Подключить 2FA", "Защита входа"],
    [13, "Смена пароля", "Аккаунт → Сменить пароль", "Безопасность"],
    [14, "Excel", "Квартиры → Скачать Excel", "Выгрузка"],
  ];
  ...
  for (let i = 6; i < 14; i++) {
    const row = i < 10 ? 0 : 1;
    const col = row === 0 ? i - 6 : i - 10;
    const x = 88 + col * 275;
    const y = row === 0 ? 606 : 654;
    addShape(slide, "roundRect", x, y, 250, 34, C.greenSoft, C.greenLine, 12);
    addText(slide, `${String(items[i][0]).padStart(2,"0")}  ${items[i][1]}`, x + 12, y + 10, 222, 14, { size: 11.5, bold: true, color: C.greenDark });
  }
...
const slides = [
  ...
  {num:8, sec:"задачи", title:"Выдача задач сотрудникам", sub:"Раздел нужен, чтобы передать конкретные замечания в работу исполнителю.", steps:[
  ...
  {num:9, sec:"квартиры", title:"Квартиры", sub:"Раздел показывает помещения, готовность, статусы осмотра, АВР и комментарии.", steps:[
  ...
  {num:10, sec:"отчет", title:"Отчет", sub:"Отчет нужен для контроля готовности объекта, статусов и общего количества работ.", steps:[
  ...
  {num:11, sec:"материалы", title:"Расход материалов", sub:"Материалы списываются на конкретную работу, чтобы сохранить историю расхода.", steps:[
  ...
  {num:12, sec:"замеры", title:"Замеры", sub:"Замеры помогают отдельно контролировать задачи по стеклу и связанным работам.", steps:[
  ...
  {num:13, sec:"аккаунт", title:"Почта и телефон", sub:"Почта и номер телефона нужны для профиля и документов, которые CRM формирует по пользователю.", steps:[
  ...
  {num:14, sec:"аккаунт", title:"Подключение 2FA", sub:"Двухэтапная аутентификация защищает вход в CRM кодом из приложения.", steps:[
    [1,"Откройте Аккаунт","Нажмите имя пользователя сверху справа и выберите Аккаунт."],
  ...
  {num:15, sec:"аккаунт", title:"Смена пароля", sub:"Пароль меняется на отдельной странице, чтобы не смешивать профиль и безопасность.", steps:[
    [1,"Откройте Аккаунт","Нажмите имя пользователя сверху справа и выберите Аккаунт."],
  ...
  {num:16, sec:"excel", title:"Excel выгрузка", sub:"Выгрузки нужны для проверки квартир, замечаний и результатов работ.", steps:[
...
const candidatePath = path.join(stagingDir, "crm-instruction-v8-candidate.pptx");
...
const result = await finalizePresentation({
  ...
  receiptPath: path.join(stagingDir, "crm-instruction-v8.validation.json"),
});
